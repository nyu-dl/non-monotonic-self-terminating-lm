...
class GPT2STHeadModel(GPT2LMHeadModel):
    ...
    def st_softmax(
        ...
        eos_token_id: int, 
        p_eos_prev: Optional[torch.FloatTensor] = None,
        return_p_eos: bool = False,
    ):
        """Self-terminating softmax
        :param torch.LongTensor logits: [batch_size, dec_seq_len, vocab_size (default = 50257)]
        :param int eos_token_id: eos token id (default = 50256)
        :param torch.FloatTensor p_eos_prev: [batch_size, 1, 1]
        :param bool return_p_eos: for decoding.
        """
        ...
        betas = torch.clamp(bound * eos_logits.sigmoid(), min=1e-20)
        ...
        if p_eos_prev is None:
            ...
            if return_p_eos:
                betas[:,:10,:] = 1
            p_eos = 1.0 - betas.log().cumsum(dim=1).exp()
            p_eos_prev = torch.zeros_like(p_eos[:,:1,:1])
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            lprobs, p_eos_prev = self.st_softmax(
                logits, 
                eos_token_id,
            ...
            next_token_lprob = lprobs[:, -1, :]
            p_eos_prev = p_eos_prev[:, -1:, :]
            next_token_logits = next_token_lprob  # use log probability as 'logits'
            # --- end change ---
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # --- begin change ---
            logits = outputs.logits
            lprobs, f_lb_lprob = self.st_softmax(
                logits, 
                eos_token_id,
                p_eos_prev=f_lb_lprob,
                return_p_eos=True
            )
            next_token_lprob = lprobs[:, -1, :]
            f_lb_lprob = f_lb_lprob[:, -1:, :]
            next_token_logits = next_token_lprob  # use log probability as 'logits'
            # --- end change ---
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            next_token_scores = next_token_lprob  # use log probability as 'logits'
            # --- end change ---
